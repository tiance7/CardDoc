...
g_type_conver = {"int": ["int.Parse"],
                 "uint": ["uint.Parse"],
                 "bool": ["Convert.ToBoolean(uint.Parse"],
                 "float":	["float.Parse"]}
...
\tpublic static %s GetData(%s, object className = null)
\t{%s
...
\tpublic static List<%s> GetData(%s, object className = null)
\t{
...
TEMPLATE_DATA_PUT2 = """
\t\t\tData.Add(template.%s + %s, template);
"""
    ...
    def _generate_template_data_get(self, template_name, template_data_file):
        ...
        if length > 1:
            ...
            for k in self.key_list.keys()[1:]:
                param = param + " ," + self.key_list[k][1] + " " + self.key_list[k][0]
        ...
        if self.key_type == "list":
            ...
            template_data_file.write(TEMPLATE_GET_DATA_BEGIN1 %
                                    (template_name, param, key_add, key, template_name,
