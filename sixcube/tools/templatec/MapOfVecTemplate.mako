...
var (
	...
	_${T_NAME}_table     map[${xmldef.get_key_type()}]${T_NAME}Vec
    _${T_NAME}_load      bool
)

func init() {
	_${T_NAME}_table = make(map[${xmldef.get_key_type()}]${T_NAME}Vec)
...
func Load${T_NAME}(xmlfile string, dataVer uint32) error {
	...
	for i, _ := range _${T_NAME}_templates.Entries {
		...
		if _, ok := _${T_NAME}_table[tplt.${xmldef.get_key()}]; ok == false {
			_${T_NAME}_table[tplt.${xmldef.get_key()}] = make(${T_NAME}Vec, 0)
		}

		if _, ok := _${T_NAME}_table[tplt.${xmldef.get_key()}]; ok {
			_${T_NAME}_table[tplt.${xmldef.get_key()}] = append(_${T_NAME}_table[tplt.${xmldef.get_key()}], &tplt)
		}
	}
...
func Find${T_NAME}Vec(${xmldef.get_key(True)} ${xmldef.get_key_type()}) ${T_NAME}Vec {
	if vec, ok := _${T_NAME}_table[${xmldef.get_key(True)} ]; ok {
		return vec
	}
	return nil
}
