...
class CSharpGenerator:
    ...
    def GenerateMessageDef(self, node):
        ...
        if self.NeedConstructor(node):
            printer.AppendLine("public sealed partial class %s : IProtocol" % node.value)
        else:
            printer.AppendLine("public sealed partial class %s" % node.value)
        printer.AppendLine("{")
        printer.IncIndent()       
    ...
    def DetermineReadMethod(self, field_type, field_name):
        if self.IsEnumType(field_type):
            return "%s = reader.ReadUInt32();" % field_name
        elif self.IsClassType(field_type):
...
def Main():
    input_path = "D:\Kindom\Kindom\Assets\Tools\Protocol\input"
    output_path = "D:\Kindom\Kindom\Assets\Tools\Protocol\output"
    if len(sys.argv) >= 3:    
