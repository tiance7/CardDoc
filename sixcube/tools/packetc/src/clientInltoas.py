...
def Transform(filename, out):
    ...
    for line in lines:
        ...
        if what != None:
            ...
            printer.AppendLine('public var %s:Object = new Object; //%s %s;'
        ...
        if what != None:
            ...
            comment = GetComment(line)
            assIndex = index + 1
            printer.AppendLine('public static const %s:String = "%s"; // %s'
                      % (msgName, assIndex, comment.decode("gbk").encode("utf8")))
            index = assIndex
            continue
    ...
    printer.IncIndent()
    printer.AppendLine("public function Message(type:String, bubbles:Boolean=false, cancelable:Boolean=false)")
    printer.AppendLine("{")
    printer.IncIndent()
